...
import { ChevronLeft, Lock, KeyRound, CheckCircle2, Send, RotateCcw } from "lucide-react-native";
...
export default function ChangePasswordScreen() {
  ...
  const handleRequestCode = async () => {
    ...
    try {
      ...
      setStep("verify");
      showModal("success", "Code Sent!", "A verification code has been sent to your email.");
    } catch (error: any) {
      console.error("Failed to send code:", error);
      showModal("error", "Error", error.response?.data?.message || "Failed to send verification code.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async () => {
    ...
    if (newPassword !== confirmPassword) {
      showModal("error", "Error", intl.formatMessage({ id: 'auth.error.passwordMismatch' }));
      return;
    }
            ...
            {step === "request" ? (
              <View className="items-center mt-10">
                ...
                <Text className="text-muted-foreground text-center mb-10 text-base leading-6 px-4">
                  For your security, we will send a 6-digit verification code to your email address before you can change your password.
                </Text>
              ...
              <View>
                ...
                <View className="mb-8">
                  <Text className="text-xs font-bold text-muted-foreground uppercase tracking-wider mb-2 ml-1">
                    {intl.formatMessage({ id: 'auth.register.confirmPassword.label' })}
                  </Text>
                  <View className="flex-row items-center bg-card rounded-2xl px-4 py-3 border border-border">
                    <Lock size={20} color={isDark ? "#94a3b8" : "#64748b"} />
                    <TextInput
